...
pub(crate) fn map_engine_error(error: ArchiveError) -> CommandErrorDto {
    match error.kind {
        ...
        ErrorKind::PasswordRequired => CommandErrorDto::password_required(
            error.message,
            Some("Enter the archive password to view or extract its contents.".to_string()),
        ),
