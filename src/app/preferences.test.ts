...
describe("preferences helpers", () => {
  ...
  it("saves non-sensitive preference fields and removes blank custom output", () => {
    ...
    expect(Object.fromEntries(storage.values)).toEqual({
      ...
      "zmanager.tableColumnOrder": "name,size,compressedSize,modified,mode,created,accessed,attributes,encrypted,method,crc,block,comment,kind,ratio,solid,linkTarget,metadataDiagnostics",
