...
test("short Disposable Task windows never clip the progress card", async ({ page }) => {
  // 1242x904 at DPR 2 — the compact logical window size where the progress card
  // previously absorbed all grid compression (its overflow-hidden zeroed the
  // automatic minimum size) and clipped the metric tiles.
  await page.setViewportSize({ width: 621, height: 452 });
  await page.goto(
    "/?surface=disposable-task&jobId=e2e-short-failed-task&kind=zipExtract&status=failed&createdAt=2026-08-11T00%3A00%3A00Z",
  );
  ...
  const layout = await page.evaluate(() => {
    const card = Array.from(
      ...
      element.textContent?.includes("Files") && element.textContent?.includes("Processed")
    );
    const content = document.querySelector<HTMLElement>("[data-task-content]");
    if (!card || !content) {
      throw new Error("Progress card markers are missing.");
    }

    const cardRect = card.getBoundingClientRect();
    const tileOverflow = Array.from(card.querySelectorAll(":scope > div:last-child > div"))
      .map((tile) => tile.getBoundingClientRect().bottom - cardRect.bottom)
    ...
    return {
      cardClipped: card.scrollHeight - card.clientHeight,
      tileOverflow,
      contentOverflowY: window.getComputedStyle(content).overflowY,
      scrollOwners,
    };
  ...
  expect(layout.cardClipped).toBeLessThanOrEqual(2);
  expect(layout.tileOverflow).toBeLessThanOrEqual(2);
  expect(layout.contentOverflowY).toMatch(/auto|scroll/);
  expect(layout.scrollOwners).toEqual(["task-content"]);
});
